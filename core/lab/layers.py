...
class Layer(ABC):
  ...
  - A 'backward' method that will be called before the 'update' method
    - Args:
      - params (dict) : weight_matrix (including bias and parametric values) for the whole layer
      - inputs (jnp.ndarray) : the input to the layer
  ...
  - A 'update' method that will be called if provided, otherwise NetLab/NetFlash will ignore the layer
    - Args:
  ...
  def __init__(self):
    """
    An Layer class is required to have the following:
